"""Fixed-plan full-API comparison of two fused32 layouts against an old shader."""
...
def select(runtime, kernel):
    original = runtime._gated4

    def gated(buffers, *, rows, cols, k):
        if rows in (128, 160, 256, 512) and (cols, k) == (3072, 1024):
            group = 512 if kernel == KERNELS[1] else 256
            runtime._dispatch(
                kernel,
                buffers[:8],
                threads=(rows // 32) * (cols // 32) * group,
                group_size=group,
                rows=rows,
                cols=cols,
                k=k,
...
def old_route(runtime):
    ...
    def dispatch(kernel, buffers, **params):
        if kernel in KERNELS:
            kernel = "gated4_16x32_k64"
            params["group_size"] = 256
            params["threads"] = (params["rows"] // 16) * (params["cols"] // 32) * 256
...
def main():
    ...
    payload = {
        ...
        "memory": [],
        "conditions": (
            "Fixed independent plans; old shader baseline; all label permutations; "
    ...
    try:
        ...
        with ExitStack() as stack:
            ...
            routes = {
                ...
                **{k: models[k] for k in args.kernels},
            }
            cases = [[n] for n in args.lengths] + [[33] * 4, [16], [24], [3, 7, 10], [159, 160]]
            if args.reverse:
                cases.reverse()
            for lengths in cases:
                    ...
                    for name in order:
                        ...
                        for kernel in KERNELS:
                            assert diff["dispatches"].get(kernel, 0) == (
                                targets * 28 if name == kernel else 0
                            )
                        np.testing.assert_array_equal(value, refs["baseline_a"])
                ...
                row = {
                    ...
                    "speedup": {k: baseline / float(np.median(timings[k])) for k in args.kernels},
