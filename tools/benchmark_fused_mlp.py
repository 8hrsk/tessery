"""Compare fused uint4 gate/up/SiLU with the complete three-operation baseline."""
...
from fused_mlp_experiment import KERNELS, experimental_runtime, fused, hashes
        ...
        with rt.command():
            for _ in range(repeats):
                if name.startswith("baseline"):
                    ...
                    rt._dispatch("silu_gate", [out, up], threads=m * n, n=m * n)
                else:
                    fused(rt, name, [*buffers, out], rows=m, cols=n, k=k)
...
def main():
    ...
    parser.add_argument("--rows", type=int, nargs="+", default=[16, 48, 128, 256, 512])
    ...
    if (
        ...
        or any(m < 16 or m > 4096 or m % 16 for m in args.rows)
        or len(set(args.rows)) != len(args.rows)
    ):
        parser.error("samples 3..100; repeats 1..100; unique rows divisible by 16, up to 4096")
    ...
    payload = {
        ...
        "conditions": (
            ...
            "GPU time per complete MLP gate/up/SiLU sequence; "
    ...
    try:
        ...
        assert (
            payload["source_hashes"] == source_hashes() and payload["experiment_hashes"] == hashes()
        )
