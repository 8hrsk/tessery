"""Independent numerical checks for the private fused MLP experiment."""
...
@pytest.mark.parametrize("rows", [16, 48, 128, 512, 4096])
@pytest.mark.parametrize("pattern", ["integer", "cancellation", "saturation"])
def test_fused_gate_up_independent_reference_and_guards(monkeypatch, rows, pattern):
    monkeypatch.syspath_prepend(str(Path(__file__).resolve().parents[2] / "tools"))
    from benchmark_fused_mlp import measure
    from fused_mlp_experiment import experimental_runtime
    ...
    args = SimpleNamespace(validate_only=True)
    with experimental_runtime() as rt:
        result = measure(rt, arrays, x, args, 2302)
        assert result["exact_baseline_equality"] and result["output_guard_untouched"]
